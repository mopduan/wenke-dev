 ...
 * @returns {Promise<any>}
 */
module.exports = function (dir, config = {}, dev = false) {
    console.log('start css compile')
    ...
    for (let i = 0; i < global.sassCompileList.length; i++) {
        ...
        sassCompilationPromises.push(new Promise(async (resolve, reject) => {
            sass.render({
                file: _sourceFile,
                includePaths: [join(__dirname, 'common-scss'), join(process.cwd(), 'static/src/ued/new_baike/pc/src/css/sprite')],
