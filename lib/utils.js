...
function verifyVersion(successCallback, failCallback) {
    http.get('http://registry.npmjs.org/wenke-dev', function (res) {
        ...
        res.on('end', function () {
            //检测本地版本与最新版本是否一致
            let fileContent = fs.readFileSync(process.cwd() + '/package.json').toString();
            let localVersion = fileContent && JSON.parse(fileContent).version;
            let latestVersion = data && JSON.parse(data)['dist-tags'].latest;
            if (localVersion && latestVersion && localVersion === latestVersion) {
                typeof successCallback === 'function' && successCallback();
            } else {
                // console.log('警告：wenke-dev工具当前版本（' + localVersion + '）不是最新版本，建议升级到最新版本(' + latestVersion + ')');
                console.log(`
 **************************************************************
 *                                                            *
 *   .=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-.       *
 ...
 **************************************************************
 `);
        ...
        throw new Error('verifyVersion Error! URL: ', url);
    });
}
