...
module.exports = function (options) {
	const { webappDirectory, commonLibraryDirectory, preact, react, hmr } =
		options;
	...
	webappDirectoryList.forEach(function (webappDirectoryPath) {
		...
		ssrEntryMap[tplKey] = {};
		utils
			.getAllFilesByDir(templateViewSrcPagePath, ['.njk', '.js'])
			.forEach(tplPath => {
				if (path.extname(tplPath.toLowerCase()) === '.js') {
					const tplPathWithoutPrefix = tplPath.replace(
						templateViewSrcPagePath,
						''
					);
					const ssrEntryKey = 'deploy' + tplPathWithoutPrefix;
