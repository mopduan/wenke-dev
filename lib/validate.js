...
exports = module.exports = function (options) {
    ...
    const webappDirectoryList = []; //所有的 webapp 工程
    const jsCompileList = []; //待编译的 js 文件集合
    const tsCompileList = []; //待编译的 ts 文件集合
    const cssCompileList = []; //模板中引用的所有 css 文件集合
    ...
    webappDirectoryList.forEach(function (item) {
        ...
        const templateViewSrcPagePath = options.np ? path.join(item, 'views/src') : path.join(item, '/src/main/webapp/WEB-INF/view/src/');
        ...
        templateFileList = templateFileList.concat(utils.getAllFilesByDir(templateViewSrcPagePath, ['.vm', '.html', '.tpl', '.njk']));
    });
    ...
    templateFileList.forEach(function (tplPath) {
        ...
        tplContent.replace(utils.getRegexpScriptElements(), function ($1, $2) {
            ...
            if ($2.toLowerCase().indexOf('release="false"') > -1 && $2.toLowerCase().indexOf('bundle') === -1) {
                return $1;
            }
