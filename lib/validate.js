...
module.exports = function (options) {
	...
	webappDirectoryList.forEach(webappDirectoryPath => {
		...
		if (!fs.existsSync(path.join(staticFilesDirectory, 'src'))) {
			throw new ValidationError(
				"can't find 'src' directory in staticDirectory " + staticFilesDirectory
			);
		}
	});
	...
	const entryList = {};//待编译的js文件列表，按webapp工程进行聚合
	webappDirectoryList.forEach(function (webappDirectoryPath) {
		...
		const staticJSSrcDirectory = path.join(staticDirectory, global.srcPrefix, 'js');
					...
					function ($2_1, $src) {
						if ($src && global.localStaticResourcesPrefix.test($src)) {
							...
							if ($src.includes('bundle')) {
								...
								entryList[tplKey][path.join(jsDeployPath, 'bundle')] = jsSrcPath;
							}
						}
					}
				);
	...
	console.log('js files to compile');
