...
function getDate() {
    ...
    return date.getFullYear().toString() + (date.getMonth() < 9 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1) + (date.getDate() < 10 ? '0' + date.getDate() : date.getDate()) + '_' + (date.getHours() < 10 ? '0' + date.getHours() : date.getHours()) + '_' + (date.getMinutes() < 10 ? '0' + date.getMinutes() : date.getMinutes()) + '_' + (date.getSeconds() < 10 ? '0' + date.getSeconds() : date.getSeconds());
}
...
function getRegexpImgSrcsetElements() {
    return /<img[\S\s\W\w]+?srcset="([\S\s]+?)"[\S\s\W\w]+?/ig;
}
...
function getRegexpBackgroundImage() {
    ...
    return /backgroundImage[\s]*:[\s]*'url\(['|"]??(\/static[\w\W]+?)['|"]??\)/ig;
}
...
function walkThroughProperties(props) {
    ...
    for (let i = 0; i < props.length; i++) {
        ...
        if (node.type === 'Property' && node.key && node.value) {
            // console.log(node);
            if ((node.key.type === 'Literal' && node.key.value === 'crossOrigin') || (node.key.type === 'Identifier' && node.key.name === 'crossOrigin')) {
                hasCrossOrigin = true;
            }

            if ((node.key.type === 'Literal' && node.key.value === 'testonly') || (node.key.type === 'Identifier' && node.key.name === 'testonly')) {
                testOnlyIdx.push(i, node.key.start, node.value.end);
            }

            if ((node.key.type === 'Literal' && node.key.value === 'md5') || (node.key.type === 'Identifier' && node.key.name === 'md5')) {
                md5Idx.push(i, node.key.start, node.value.end);
            }

            if ((node.key.type === 'Literal' && node.key.value === 'compile') || (node.key.type === 'Identifier' && node.key.name === 'compile')) {
                compileIdx.push(i, node.key.start, node.value.end);
            }

            if ((node.key.type === 'Literal' && node.key.value === 'src') || (node.key.type === 'Identifier' && node.key.name === 'src')) {
                src = node.source();
            }
        }
    }
        ...
        if (node.type === 'ObjectExpression') {
            ...
            for (let i = 0; i < walkThrough.indices.length; i++) {
                ...
                if (idx.length === 3) {
                    ...
                    if (walkThrough.src.indexOf('reportAjax/main.js') > -1 && i === 0) { // reportAjax在测试环境时需要替换
            ...
            if (!walkThrough.hasCrossOrigin && walkThrough.src.indexOf('cache.soso.com') > -1)
                node.properties[node.properties.length - 1].update('"crossOrigin": "anonymous", ' + node.properties[node.properties.length - 1].source());
        }

        let _source = node.source();
        for (let i = 0; i < replacedArray.length; i++) {
            _source = _source.replace(replacedArray[i].src, replacedArray[i].sub);
        }
...
function manipulateScript(content, regexp, env, taskid, vid) {
    return content.replace(regexp, function (createElement, srcObjStr) {
        ...
        // 补全对象中的
        const validStr = srcObjStr.replace(/(['"])?([a-zA-Z0-9_]+)(['"])?:/g, function ($key, $1, $2) {
            if ($2 !== 'http' && $2 !== 'https') {
                return `"${$2}": `;
            } else {
                return $key;
        ...
        try {
            if (srcObjStr.indexOf('testonly') > -1) {
                if (env === 'test') {
                    return createElement.replace(srcObjStr, manipulateScriptAttribute(srcObjStr, env, taskid, vid));
                } else {
                    return '';
                }
            } else {
