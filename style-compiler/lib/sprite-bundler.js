...
async function spriteBundler(params) {
	...
	assert(
		imgName,
		'An `imgName` parameter was not provided to `gulp.spritesmith` (required)'
	);
	assert(
		cssName,
		'A `cssName` parameter was not provided to `gulp.spritesmith` (required)'
	);
	...
	if (retinaSrcFilter || retinaImgName) {
		assert(
			...
			'Retina settings detected. We must have both `retinaSrcFilter` and ' +
	...
	if (Array.isArray(images) && images.length) {
			...
			if (images.length !== retinaImages.length) {
				const err = new Error(
					'Retina settings detected but ' +
						...
						'We have ' +
						images.length +
						' normal images and expect these numbers to line up. ' +
		...
		const bundler = async function () {
			...
			const generateRetinaSpritesheet = new Promise((resolve, reject) => {
				if (retinaImages) {
					retinaSpritesmith.createImages(
						retinaImages,
						(error, _retinaImages) => {
							if (error) {
								reject(error);
			...
			// 格式：[ [], [] ]
			const _sprites = await Promise.all([
				generateNormalImages,
				generateRetinaSpritesheet
			]);
			...
			Object.getOwnPropertyNames(coordinates)
				.sort()
			...
			if (retinaResult) {
				...
				if (intersectingNames.length) {
					throw new Error(
						'Normal and retina sprites have same names: ' +
							JSON.stringify(intersectingNames) +
							'. ' +
				...
				retinaGroups = cleanCoords.map(function getRetinaGroups(
					normalSprite,
					i
				) {
					// Generate our group
			...
			if (handlebarsHelpers) {
				Object.keys(handlebarsHelpers).forEach(function registerHelper(
					...
					templater.registerHandlebarsHelper(
						helperKey,
						handlebarsHelpers[helperKey]
					);
				});
			}
			...
			if (cssTemplate) {
				if (typeof cssTemplate === 'function') {
					...
					templater.addHandlebarsTemplate(
						cssFormat,
						fs.readFileSync(cssTemplate, 'utf8')
					);
				}
			...
			if (retinaResult) {
				_outputStreamPromises.push(
					new Promise((resolve, reject) => {
						const _writableStream = fs.createWriteStream(
							path.join(dest, retinaImgName)
						);
			...
			_outputStreamPromises.push(
				new Promise((resolve, reject) => {
					// Output the CSS
					customizeFile.mkdirRecursive(
						path.dirname(path.join(dest, cssName))
					);

					const _cssOutputStream = fs.createWriteStream(
						path.join(dest, cssName)
					);
