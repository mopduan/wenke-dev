...
function parseSprites(sprites) {
	const data = sprites.reduce((result, sprite) => {
		...
		const spriteName = escaped_image.substring(
			escaped_image.lastIndexOf('/') + 1
		);
...
module.exports = function (options) {
	...
	function handler(data) {
		...
		scss.push(
			`$__sprite-is-rem__: ${Boolean(rem && data.retina_sprites)};`
		);
		scss.push(`$__sprite-only-retina__: ${Boolean(!divideBy2 && retina)};`);
		scss.push(
			`$__sprite-local-group__:  ${parseSprites(
				data.sprites,
				dirPrefix
			)};`
		);
		if (data.retina_sprites) {
			scss.push(
				`$__sprite-local-group-2x__: ${parseSprites(
					data.retina_sprites,
					dirPrefix
				)};`
			);
		...
		scss.push(
			fs
				.readFileSync(path.join(__dirname, 'common-scss/function.scss'))
				.toString()
		);
